...
//     printf("%f %f %d %d %d\n", ker[b_id*k_size*k_size*n_c], image[(tid_y)*im_size + tid_x], tid_x, tid_y, b_id);
//     printf("%f %d %d %d\n", out[b_id*out_size*out_size + tid_y*(out_size) + tid_x], tid_x, tid_y, b_id);
...
float * Flatten(float *in, int H, int W, int n_c){
    float *out = (float *) malloc(sizeof(float) * H * W * n_c);
    for(int i = 0; i<H; i++){
        for(int j = 0; j<W; j++){
            for(int k = 0; k < n_c; k++){
                out[i*W*n_c + j*n_c + k] = in[k*(W * H) + j*H + i];
            }
        }
    }
